...
class DroneServer(DatagramProtocol):
	...
	self.loopCall = task.LoopingCall(self.executeRotate)
        self.loopCall.start(1.0) # call every second
    def stopRotateDevice(self):
        self.loopCall.stop()
    def holdRotateDevice(self):
        self.rotateIsHolding = True
	
    #timer = threading.Timer(1.0, executeRotate,self)    
    ...
    def datagramReceived(self, datagram, (host, port)):
        ...
        if udpObj['Command'] == "Configure":
            ...
            print "Configure - step 2" 
            nois_global_variable.getAnalog_sig_source().set_sampling_freq(float(self._samp))
            nois_global_variable.getBlocks_throttle().set_sample_rate(float(self._samp))
            nois_global_variable.getWxgui_fftsink().set_sample_rate(float(self._samp))
...
class Usrp_collect_block(grc_wxgui.top_block_gui):
    droneServer = None
    def __init__(self,droneServer = None):
        self.droneServer = droneServer;
        grc_wxgui.top_block_gui.__init__(self, title="FMS Usrp")
        ...
        self.notebook_0.AddPage(grc_wxgui.Panel(self.notebook_0), "Water")
        ...
        self.analog_sig_source_x_0 = analog.sig_source_c(samp_rate, analog.GR_SIN_WAVE, 105.3e6, 1, 0)        
        self.wxgui_fftsink2_0 = fftsink2.fft_sink_c(
        	...
        	peak_hold=False,
            analog_sig_source = self.analog_sig_source_x_0,
		    blocks_throttle =  self.blocks_throttle_0,
        )
        nois_global_variable.setAnalog_sig_source(self.analog_sig_source_x_0)
        nois_global_variable.setBlocks_throttle(self.blocks_throttle_0)    
    ...
    def callBackWhenHandleMessage(self,message):
        ...
        #listNumber = np.array(message).tolist()
	#print listNumber



def initGnuThread(ucb=Usrp_collect_block, options=None):
